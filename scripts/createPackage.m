function createPackage(options)
  ...
  PREFIX = "[createPackage]";

  if options.DryRun
    fprintf("%s DRY RUN enabled: build and package steps are skipped.\n", PREFIX);
    return;
  end
  ...
  if status ~= 0
    error("%s Error building documentation with Sphinx:\n%s", PREFIX, cmdout);
  end
  ...
  if options.PackageToolbox
    ...
    fprintf("%s Toolbox packaged using %s\n", PREFIX, projectFile);
  else
    fprintf("%s Packaging skipped by request.\n", PREFIX);
  end
