function [zgrid,xgrid,ygrid] = RegularizeData3D_mod(x,y,z,xnodes,ynodes,varargin)
...
if (params.tilesize < max(nx,ny))
  ...
  % derivative, not a first derivative.  This is an important distinction because "gradient" implies a horizontal
	...
	smoothnessScale_y = sqrt(FidelityEquationCount / (nx*(ny-2)));
    axisScale_y = (max(ynodes(:))-min(ynodes(:))).^2;

	Areg =  sparse(repmat(1:nx*(ny-2),1,3), [ind-1, ind, ind+1], smoothnessScale_y*axisScale_y*xyRelativeStiffness(2)*[-2./(dy1.*(dy1+dy2)), 2./(dy1.*dy2), -2./(dy2.*(dy1+dy2))], nx*(ny-2), ngrid);
	...
	smoothnessScale_x = sqrt(FidelityEquationCount / ((nx-2)*ny));
    
    axisScale_x = (max(xnodes(:))-min(xnodes(:))).^2;

	Areg = [Areg;sparse(repmat(1:(nx-2)*ny,1,3),[ind-ny,ind,ind+ny], smoothnessScale_x*axisScale_x*xyRelativeStiffness(1)*[-2./(dx1.*(dx1+dx2)), 2./(dx1.*dx2), -2./(dx2.*(dx1+dx2))],(nx-2)*ny,ngrid)];
	nreg = size(Areg,1);
